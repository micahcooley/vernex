"""
Vernex Chat, Tool Calling & Web Search Fine-tuning
Builds on base model checkpoint
"""
import torch
...
def train():
    ...
    print(f"Training Chat/Tools/Search on: {device}")
    ...
    corpora = [
        {'path': DATA_DIR / "conversation_corpus.txt", 'weight': 5},
        {'path': DATA_DIR / "tool_corpus.txt", 'weight': 10},
    ]
    ...
    print("Starting Chat/Tools/Search Fine-tuning...")
    for epoch in range(3):
        ...
        for batch_idx, (x, y) in enumerate(loader):
            ...
            if batch_idx % 100 == 0:
                tmp_path = MODEL_DIR / "vernex_chat_tools_latest.pt.tmp"
                final_path = MODEL_DIR / "vernex_chat_tools_latest.pt"
    ...
    # Save final and copy to base for next stage
    torch.save(model.state_dict(), str(MODEL_DIR / "vernex_chat_tools_final.pt"))
    torch.save(model.state_dict(), str(MODEL_DIR / "vernex_nano_latest.pt"))
    print("Chat/Tools/Search training complete!")

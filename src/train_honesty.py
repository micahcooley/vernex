...
def train():
    ...
    print(f"Training Honesty on: {device}")
    ...
    corpora = [
        {'path': DATA_DIR / "honesty_corpus.txt", 'weight': 20},
        {'path': DATA_DIR / "conversation_corpus.txt", 'weight': 2},
    ]
    ...
    print("Starting Honesty Fine-tuning...")
    for epoch in range(5):
        ...
        for batch_idx, (x, y) in enumerate(loader):
            ...
            if batch_idx % 100 == 0:
                tmp_path = MODEL_DIR / "vernex_honesty_latest.pt.tmp"
                final_path = MODEL_DIR / "vernex_honesty_latest.pt"
    ...
    torch.save(model.state_dict(), str(MODEL_DIR / "vernex_honesty_final.pt"))
    torch.save(model.state_dict(), str(MODEL_DIR / "vernex_nano_latest.pt"))
    print("Honesty training complete!")
